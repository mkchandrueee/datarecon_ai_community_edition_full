...
| [0011](0011-metadata-driven-sql-generation.md) | Validation SQL is generated from the database's own catalog rather than a language model — it cannot reference a column that doesn't exist, works offline, and avoids the COUNT(*) trap that would make Record Count report 1 |
| [0012](0012-referential-integrity-validation.md) | Referential Integrity checks orphans over arbitrary column pairs on arbitrary connections rather than only declared foreign keys — the defects worth finding are where the constraint isn't enforced; NULL foreign keys are excluded, not counted as orphans |
